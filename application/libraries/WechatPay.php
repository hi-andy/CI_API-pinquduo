 ...
 * Date: 2017/9/5
 * Time: 15:37
 * Notice: 只需要将上面下载好的 CA 证书放置到您的服务器上某个位置，然后修改 php.ini 的 curl.cainfo 为该路径（绝对路径！），重启 php-fpm 服务即可。curl.cainfo = /path/to/downloaded/cacert.pem
 */
...
//        }
        if ($result->return_code == 'SUCCESS' && $result->result_code == 'SUCCESS'){
            ...
            $config = $payment->configForAppPayment($prepayId);//生成APP支付配置  数组格式 可以返回给APP调用
        }
    }
    ...
    public function jsOrder(){
        ...
        if ($result->return_code == 'SUCCESS' && $result->result_code == 'SUCCESS'){
            $prepayId = $result->prepay_id;
            $json = $payment->configForPayment($prepayId); // 返回 json 字符串，如果想返回数组，传第二个参数 false WeixinJSBridge前端模式 TODO 二选一
            $config = $payment->configForJSSDKPayment($prepayId); // 返回数组  JSSDK前端模式 TODO 二选一
        }
    }
